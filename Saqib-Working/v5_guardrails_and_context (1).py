...
import grpc.aio # Import grpc.aio for explicit shutdown
...
model_name = os.getenv('MODEL') # Read the model name from .env
if not model_name:
...
budget_analysis_agent = Agent(
    ...
    model=model_settings # Use ModelSettings object
)
...
async def main():
    # Initialize gRPC async IO integration
    grpc.aio.init_grpc_aio()
    print("gRPC async IO initialized.")
    ...
    # No explicit shutdown needed here if init_grpc_aio() is used;
    # Cleanup should happen automatically when the event loop closes.
    # await grpc.aio.shutdown() # Removed incorrect call
